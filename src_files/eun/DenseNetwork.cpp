...
nn::DenseInput *nn::DenseNetwork::getInputLayer() {
    return input;
}

nn::DenseLayer *nn::DenseNetwork::getHiddenLayer(int index) {
    return layers[index];
}
...
void nn::DenseNetwork::copyWeights(nn::DenseNetwork *other) {
    memcpy(getInputLayer()->getWeights()->value,
           other->getInputLayer()->getWeights()->value,
           this->getInputLayer()->getWeights()->size * sizeof(float));
    memcpy(getInputLayer()->getBias()->value,
           other->getInputLayer()->getBias()->value,
           this->getInputLayer()->getBias()->size * sizeof(float));

    for (int i = 0; i < hiddenCount; i++) {
        memcpy(getHiddenLayer(i)->getWeights()->value,
               other->getHiddenLayer(i)->getWeights()->value,
               this->getHiddenLayer(i)->getWeights()->size * sizeof(float));
        memcpy(getHiddenLayer(i)->getBias()->value,
               other->getHiddenLayer(i)->getBias()->value,
               this->getHiddenLayer(i)->getBias()->size * sizeof(float));
    }

    memcpy(getOutputLayer()->getWeights()->value,
           other->getOutputLayer()->getWeights()->value,
           this->getOutputLayer()->getWeights()->size * sizeof(float));
    memcpy(getOutputLayer()->getBias()->value,
           other->getOutputLayer()->getBias()->value,
           this->getOutputLayer()->getBias()->size * sizeof(float));
}
