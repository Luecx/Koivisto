...
void main_tune_features() {
    ...
    loadPositionFile("E:/a.book", 10000000);
    loadPositionFile("E:/b.book", 10000000);
    loadPositionFile("E:/c.book", 10000000);
    ...
    for (int i = 0; i < 100; i++) {
        ...
        tuning::optimisePSTBlackBox(K, reinterpret_cast<EvalScore*>(piece_opp_king_square_table[4]), 225, 100, 3, 16);
        ...
        for (int s = 0; s < 225; s++) {
            if (s % 15 == 0)
                std::cout << std::endl;
            std::cout << "M(" << setw(5) << MgScore(piece_opp_king_square_table[4][s]) << "," << setw(5)
                      << EgScore(piece_opp_king_square_table[4][s]) << "), ";
        }
        std::cout << std::endl;
        //        std::cout << std::endl;
