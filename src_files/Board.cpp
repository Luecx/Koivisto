...
/**
 * The default constructor uses a fen-representation of the board. if nothing is specified, the
 * starting position will be used. This might crash if the given fen is illegal in its structure. e.g.
 * not all rows/columns specified.
 * @param fen
 */
Board::Board(std::string fen) {
    ...
    // note that we do not read information about move counts. This is usually not required for
    // playing games.
}
...
/**
 * Returns a FEN-representation of the board object which can be used for other engines, and
 * debugging.
 */
...
bool Board::isInCheck(Color player) {
    // we do this by casting rays from the king position and checking if an opponent piece is on those
    // rays which could attack the given ray.
    if (player == WHITE) {
 ...
 * @param piece
 */
template<bool updateNN> void Board::setPiece(Square sq, Piece piece) {
 ...
 * @param sq
 */
template<bool updateNN> void Board::unsetPiece(Square sq) {
 ...
 * @param piece
 */
template<bool updateNN> void Board::replacePiece(Square sq, Piece piece) {
...
template<Color attacker> bool Board::isUnderAttack(Square square) {
    ...
    if constexpr (attacker == WHITE) {
        return (lookUpRookAttack(square, m_occupiedBB)
                & (m_piecesBB[WHITE_QUEEN] | m_piecesBB[WHITE_ROOK]))
                   != 0
               || (lookUpBishopAttack(square, m_occupiedBB)
                   & (m_piecesBB[WHITE_QUEEN] | m_piecesBB[WHITE_BISHOP]))
                      != 0
        ...
        return (lookUpRookAttack(square, m_occupiedBB)
                & (m_piecesBB[BLACK_QUEEN] | m_piecesBB[BLACK_ROOK]))
                   != 0
               || (lookUpBishopAttack(square, m_occupiedBB)
                   & (m_piecesBB[BLACK_QUEEN] | m_piecesBB[BLACK_BISHOP]))
                      != 0
...
bool Board::isLegal(Move m) {
    ...
    if (isEnPassant(m)) {
        ...
        this->move(m);
        bool isOk = (bb::lookUpRookAttack(thisKing, m_occupiedBB)
                     & (opponentQueenBitboard | opponentRookBitboard))
                        == 0
                    && (bb::lookUpBishopAttack(thisKing, m_occupiedBB)
 ...
 * It assumes that this is mainly called for moves coming from the hash table which have been
 * generated by the move generator. This means that moves like pawn from a2 to c3 will be alright if
...
bool Board::isPseudoLegal(Move m) {
    ...
    switch (getPieceType(pieceFrom)) {
        case PAWN:
            if (isPromotion(m)) {
                ...
                return getPiece(sqTo) == -1
                       && getPiece(sqTo + 8 * (getActivePlayer() == WHITE ? -1 : 1)) == -1;
            } else {
        ...
        // for sliding pieces we only need to validate there is no piece on the squares in between the
        // from and to square
        case BISHOP:;
        case ROOK:;
        case QUEEN:
            // if there is a piece in the way, this move is invalid
            if ((IN_BETWEEN_SQUARES[sqFrom][sqTo] & m_occupiedBB) != 0)
                return false;
        ...
        case KING:
            ...
            if (isCastle(m)) {
                ...
                U64  canCastleMask =
                    m_occupiedBB
                    & (activePlayer == WHITE ? (kingSideCastle ? CASTLING_WHITE_KINGSIDE_MASK
                                                               : CASTLING_WHITE_QUEENSIDE_MASK)
                                             : (kingSideCastle ? CASTLING_BLACK_KINGSIDE_MASK
    ...
    // finally we need to check for the moves if its a capture and if thats the case, that the
    // captured piece is on that square
    if (isCapture)
        return getPiece(sqTo) == getCapturedPiece(m);
    return getPiece(sqTo) == -1;
}
