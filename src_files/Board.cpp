...
bool Board::isPseudoLegal(Move m){
    ...
    // first we extract some information which are definitely required
    Square sqFrom        = getSquareFrom(m);
    Square sqTo          = getSquareTo(m);
    Piece  pieceFrom     = getMovingPiece(m);
    Color  activePlayer  = pieceFrom < 8 ? WHITE:BLACK;
    bool   isCapture     = move::isCapture(m);
    
    // check if the color is the correct side to move
    if (activePlayer != this->getActivePlayer()) return false;
    
    // check if the piece at the starting square is also the moving piece
    if(getPiece(sqFrom = getSquareFrom(m)) != pieceFrom) return false;
