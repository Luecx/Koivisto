...
void Board::move(Move m) {
    ...
    if (pFrom % 6 == PAWN) {
        ...
        // promotions are handled differently because the new piece at the target square is not the
        // piece that initially moved.
...
void Board::getPseudoLegalMoves(MoveList *moves) {
    ...
    if (getActivePlayer() == WHITE) {
        ...
        if (m_pieces[WHITE_PAWN] & RANK_7) {
            ...
            while (attacks) {
                target = bitscanForward(attacks);
                moves->add(genMove(
                    target - 7, target, QUEEN_PROMOTION_CAPTURE, WHITE_PAWN, getPiece(target)));
                moves->add(genMove(
                    target - 7, target, ROOK_PROMOTION_CAPTURE, WHITE_PAWN, getPiece(target)));
                moves->add(genMove(
                    target - 7, target, BISHOP_PROMOTION_CAPTURE, WHITE_PAWN, getPiece(target)));
                moves->add(genMove(
                    target - 7, target, KNIGHT_PROMOTION_CAPTURE, WHITE_PAWN, getPiece(target)));
                attacks = lsbReset(attacks);
            }
            ...
            while (attacks) {
                target = bitscanForward(attacks);
                moves->add(genMove(
                    target - 9, target, QUEEN_PROMOTION_CAPTURE, WHITE_PAWN, getPiece(target)));
                moves->add(genMove(
                    target - 9, target, ROOK_PROMOTION_CAPTURE, WHITE_PAWN, getPiece(target)));
                moves->add(genMove(
                    target - 9, target, BISHOP_PROMOTION_CAPTURE, WHITE_PAWN, getPiece(target)));
                moves->add(genMove(
                    target - 9, target, KNIGHT_PROMOTION_CAPTURE, WHITE_PAWN, getPiece(target)));
                attacks = lsbReset(attacks);
            }
        }
        ...
        if (m_pieces[BLACK_PAWN] & RANK_2) {
            ...
            while (attacks) {
                target = bitscanForward(attacks);
                moves->add(genMove(
                    target + 9, target, QUEEN_PROMOTION_CAPTURE, BLACK_PAWN, getPiece(target)));
                moves->add(genMove(
                    target + 9, target, ROOK_PROMOTION_CAPTURE, BLACK_PAWN, getPiece(target)));
                moves->add(genMove(
                    target + 9, target, BISHOP_PROMOTION_CAPTURE, BLACK_PAWN, getPiece(target)));
                moves->add(genMove(
                    target + 9, target, KNIGHT_PROMOTION_CAPTURE, BLACK_PAWN, getPiece(target)));
                attacks = lsbReset(attacks);
            }
            ...
            while (attacks) {
                target = bitscanForward(attacks);
                moves->add(genMove(
                    target + 7, target, QUEEN_PROMOTION_CAPTURE, BLACK_PAWN, getPiece(target)));
                moves->add(genMove(
                    target + 7, target, ROOK_PROMOTION_CAPTURE, BLACK_PAWN, getPiece(target)));
                moves->add(genMove(
                    target + 7, target, BISHOP_PROMOTION_CAPTURE, BLACK_PAWN, getPiece(target)));
                moves->add(genMove(
                    target + 7, target, KNIGHT_PROMOTION_CAPTURE, BLACK_PAWN, getPiece(target)));
                attacks = lsbReset(attacks);
            }
        }
    }
    ...
    while (queens) {
        ...
        attacks = (lookUpRookAttack(s, m_occupied) | lookUpBishopAttack(s, m_occupied)) &
                  attackableSquares;
    ...
    while (kings) {
        ...
        if (getActivePlayer() == WHITE) {
            if (getCastlingChance(STATUS_INDEX_WHITE_QUEENSIDE_CASTLING) &&
                getPiece(A1) == WHITE_ROOK && (m_occupied & CASTLING_WHITE_QUEENSIDE_MASK) == 0) {
                moves->add(genMove(E1, C1, QUEEN_CASTLE, WHITE_KING));
            }
            if (getCastlingChance(STATUS_INDEX_WHITE_KINGSIDE_CASTLING) &&
                getPiece(H1) == WHITE_ROOK && (m_occupied & CASTLING_WHITE_KINGSIDE_MASK) == 0) {
                moves->add(genMove(E1, G1, KING_CASTLE, WHITE_KING));
            }

        } else {
            if (getCastlingChance(STATUS_INDEX_BLACK_QUEENSIDE_CASTLING) &&
                getPiece(A8) == BLACK_ROOK && (m_occupied & CASTLING_BLACK_QUEENSIDE_MASK) == 0) {
                moves->add(genMove(E8, C8, QUEEN_CASTLE, BLACK_KING));
            }
            if (getCastlingChance(STATUS_INDEX_BLACK_KINGSIDE_CASTLING) &&
                getPiece(H8) == BLACK_ROOK && (m_occupied & CASTLING_BLACK_KINGSIDE_MASK) == 0) {
...
Move Board::getPreviousMove() {
    if (m_boardStatusHistory.empty()) return 0;
...
U64 Board::getLeastValuablePiece(U64 attadef, Score bySide, Piece &piece) {
    for (piece = PAWN + bySide * 6; piece <= KING + bySide * 6; piece += 1) {
        ...
        if (subset) return subset & -subset;  // single bit
    }
    return 0;  // empty set
}
...
Score Board::staticExchangeEvaluation(Move m) {
    ...
    do {
        ...
        occ ^= fromSet;
        attadef |= occ & ((lookUpBishopAttack(sqTo, occ) & bishopsQueens) |
                          (lookUpRookAttack(sqTo, occ) & rooksQueens));
...
U64 Board::attacksTo(U64 p_occupied, Square sq) {
    ...
    return ((shiftNorthWest(sqBB) | shiftNorthEast(sqBB)) & m_pieces[BLACK_PAWN]) |
           ((shiftSouthWest(sqBB) | shiftSouthEast(sqBB)) & m_pieces[WHITE_PAWN]) |
           (KNIGHT_ATTACKS[sq] & knights) | (KING_ATTACKS[sq] & kings) |
...
bool Board::isUnderAttack(Square square, Color attacker) {
    ...
    if (attacker == WHITE) {
        return (lookUpRookAttack(square, m_occupied) &
                (m_pieces[WHITE_QUEEN] | m_pieces[WHITE_ROOK])) != 0 ||
               (lookUpBishopAttack(square, m_occupied) &
                (m_pieces[WHITE_QUEEN] | m_pieces[WHITE_BISHOP])) != 0 ||
        ...
        return (lookUpRookAttack(square, m_occupied) &
                (m_pieces[BLACK_QUEEN] | m_pieces[BLACK_ROOK])) != 0 ||
               (lookUpBishopAttack(square, m_occupied) &
                (m_pieces[BLACK_QUEEN] | m_pieces[BLACK_BISHOP])) != 0 ||
...
bool Board::givesCheck(Move m) {
    ...
    if (isCastle(m)) {
        unsetBit(m_occupied, sqFrom);
        Square rookSquare = getActivePlayer() == WHITE ? (sqTo - sqFrom) > 0 ? F1 : D1
                                                       : (sqTo - sqFrom) > 0 ? F8 : D8;
...
bool Board::isLegal(Move m) {
    ...
    if (isEnPassant(m)) {
        this->move(m);
        bool isOk = (bb::lookUpRookAttack(thisKing, m_occupied) &
                     (opponentQueenBitboard | opponentRookBitboard)) == 0 &&
                    (bb::lookUpBishopAttack(thisKing, m_occupied) &
                     (opponentQueenBitboard | opponentBishopBitboard)) == 0;
        this->undoMove();
        ...
        Type t = getType(m);
        if (this->getActivePlayer() == WHITE) {
            secure = (t == QUEEN_CASTLE) ? bb::CASTLING_WHITE_QUEENSIDE_SAFE
                                         : bb::CASTLING_WHITE_KINGSIDE_SAFE;
            return (getAttackedSquares(BLACK) & secure) == 0;
        } else {
            secure = (t == QUEEN_CASTLE) ? bb::CASTLING_BLACK_QUEENSIDE_SAFE
                                         : bb::CASTLING_BLACK_KINGSIDE_SAFE;
...
BoardStatus *Board::getBoardStatus() {
    return &m_boardStatusHistory.back();
}
...
const U64 *Board::getPieces() const {
    return m_pieces;
}

const U64 *Board::getTeamOccupied() const {
    return m_teamOccupied;
}

U64 Board::getPieces(Color color, Piece piece) {
    return m_pieces[color * 6 + piece];
}
