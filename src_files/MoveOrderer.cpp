...
void MoveOrderer::setMovesPVSearch(move::MoveList *moves, move::Move hashMove, SearchData *sd, Board* board, Depth ply) {
    ...
    for(int i = 0; i < moves->getSize(); i++){
        ...
        if(sameMove(m,hashMove)){
            ...
            MoveScore mvvLVA = 100 * (getCapturedPiece(m) % 6) - 10 * (getMovingPiece(m) % 6) + (getSquareTo(board->getPreviousMove()) == getSquareTo(m));
            if(SEE >= 0){
                if(mvvLVA == 0){
...
void MoveOrderer::setMovesQSearch(move::MoveList *moves, Board *b) {
    this->moves = moves;
    this->counter = 0;
    ...
    for(int i = 0; i < moves->getSize(); i++){
        ...
        MoveScore mvvLVA = 100*(getCapturedPiece(m) % 6) - 10*(getMovingPiece(m) % 6)+(getSquareTo(b->getPreviousMove()) == getSquareTo(m));
