...
EvalScore ROOK_KING_LINE                = M(   27,    1);
EvalScore BISHOP_DOUBLED                = M(   14,   82);
EvalScore BISHOP_FIANCHETTO             = M(   -4,    2);
EvalScore BISHOP_PIECE_SAME_SQUARE_E    = M(    3,    5);
EvalScore QUEEN_DISTANCE_ENEMY_KING     = M(    0,  -19);
EvalScore KING_CLOSE_OPPONENT           = M(  -13,   22);
EvalScore KING_PAWN_SHIELD              = M(   29,    9);
EvalScore CASTLING_RIGHTS               = M(   18,    3);
EvalScore MINOR_BEHIND_PAWN             = M(    5,   25);

EvalScore mobilityKnight[9] = {
    M(  -49,   -9), M(  -38,   46), M(  -33,   78), M(  -29,   96), M(  -25,  108),
    M(  -21,  119), M(  -15,  121), M(   -6,  117), M(    8,  103), };

EvalScore mobilityBishop[14] = {
    M(  -14,  -40), M(   -4,   31), M(    3,   65), M(    8,   86), M(   14,  103),
    M(   18,  117), M(   20,  124), M(   20,  129), M(   22,  133), M(   26,  132),
    M(   33,  127), M(   48,  121), M(   58,  128), M(   76,  102), };

EvalScore mobilityRook[15] = {
    M(  -52,   73), M(  -46,  118), M(  -43,  155), M(  -42,  183), M(  -40,  198),
    M(  -34,  205), M(  -29,  212), M(  -21,  214), M(  -14,  219), M(   -8,  225),
    M(   -3,  229), M(    1,  232), M(   11,  229), M(   36,  214), M(   94,  183), };

EvalScore mobilityQueen[28] = {
    M( -189,  110), M( -173,  112), M( -161,  241), M( -157,  323), M( -155,  368),
    M( -155,  399), M( -154,  424), M( -152,  441), M( -150,  455), M( -148,  464),
    M( -146,  472), M( -144,  479), M( -143,  484), M( -143,  492), M( -144,  498),
    M( -146,  501), M( -146,  503), M( -148,  506), M( -148,  506), M( -142,  500),
    M( -135,  491), M( -133,  486), M( -139,  482), M( -124,  473), M( -187,  500),
    M(  -66,  431), M(  -97,  472), M( -170,  517), };

EvalScore hangingEval[5] = {
    M(   -3,   -0), M(   -4,   -0), M(   -6,   -6), M(   -5,   -5), M(   -4,   -9), };

EvalScore pinnedEval[15] = {
    M(   -8,   -9), M(  -13,    5), M(   -9,   44), M(  -33,  -63), M(  -30,  -13),
    M(  -22,   48), M(  -13,   -9), M(  -37,  -10), M(  -18,   38), M(  -59,  -39),
    M(   -2,   -8), M(  -16,   36), M(  -18,  -72), M(  -48,  -46), M(  -19,   40), };

EvalScore passer_rank_n[16] = {
    M(    0,    0), M(  -10,  -31), M(  -15,  -27), M(   -9,    1),
    M(   19,   28), M(   41,   85), M(   16,   51), M(    0,    0),
    M(    0,    0), M(    1,    2), M(  -31,  -61), M(  -28,  -17),
    M(  -16,  -14), M(   33,   13), M(   97, -112), M(    0,    0), };

EvalScore bishop_pawn_same_color_table_o[9] = {
    M(  -34,   40), M(  -41,   42), M(  -42,   30),
    M(  -45,   21), M(  -50,   11), M(  -55,   -2),
    M(  -58,  -20), M(  -59,  -37), M(  -69,  -85), };

EvalScore bishop_pawn_same_color_table_e[9] = {
    M(  -20,   38), M(  -38,   47), M(  -43,   39),
    M(  -49,   31), M(  -53,   19), M(  -57,    3),
    M(  -59,  -18), M(  -58,  -33), M(  -65,  -50), };

EvalScore kingSafetyTable[100] = {
    M(  -18,   -8), M(    0,    0), M(  -18,   -9), M(  -15,  -11), M(  -12,   -7),
    M(   14,  -13), M(    6,  -14), M(   25,   -8), M(   20,   -8), M(   48,  -25),
    M(   66,  -11), M(   87,  -28), M(   62,  -18), M(  116,  -21), M(  122,  -12),
    M(  136,  -23), M(  132,  -13), M(  194,  -18), M(  237,  -52), M(  245,  -54),
    M(  251,  -29), M(  215,  -18), M(  344,  -47), M(  279,    3), M(  322,    6),
    M(  314,   25), M(  416,   26), M(  421,  -27), M(  371,  100), M(  468,   40),
    M(  505, -155), M(  537,  -30), M(  660, -204), M(  722, -186), M(  545,   12),
    M( 2076,-3567), M(  110, 1203), M( 1922,-2490), M(  828, -367), M( 1032,  867),
    M( 1455,-1835), M( 1032, -464), M( 1546, 1343), M(  500,  500), M(  501,  500),
    M( 1644, 1530), M(  500,  500), M(  920,  699), M(  507,  500), M( -511, -515),
