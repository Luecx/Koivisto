...
EvalScore connected_pawns_table[8]{
    M(    0,    0),
    M(   -1,    5),
    M(    0,    5),
    M(   -1,    6),
...
EvalScore fast_connected_pawns[2][8];
...
void eval_init() {
    ...
    for(int i = 0; i < 8; i++){
        fast_connected_pawns[WHITE][i] = connected_pawns_table[i];
        fast_connected_pawns[BLACK][i] = connected_pawns_table[7-i];
    }
}
...
bb::Score Evaluator::evaluate(Board* b) {
    ...
    while (k) {
        ...
        evalScore += fast_connected_pawns[WHITE][rankIndex(square)] * ((whitePawns & whitePawnCover) != 0);
        k = lsbReset(k);
    }
    ...
    while (k) {
        ...
        evalScore -= fast_connected_pawns[BLACK][rankIndex(square)] * ((whitePawns & whitePawnCover) != 0);
        k = lsbReset(k);
    }
    k = whitePassers;
    while (k) {
