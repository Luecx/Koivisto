...
EvalScore PAWN_PASSED_SQUARE_RULE       = M(    3,   17);
...
EvalScore ROOK_KING_LINE                = M(   14,    5);
...
EvalScore QUEEN_DISTANCE_ENEMY_KING     = M(  -16,   20);
...
EvalScore SAFE_QUEEN_CHECK              = M(    8,   27);
EvalScore SAFE_ROOK_CHECK               = M(   12,    6);
EvalScore SAFE_BISHOP_CHECK             = M(    6,    5);
EvalScore SAFE_KNIGHT_CHECK             = M(   13,    4);
EvalScore PAWN_ATTACK_MINOR             = M(   42,   75);
EvalScore PAWN_ATTACK_ROOK              = M(   45,   29);
EvalScore PAWN_ATTACK_QUEEN             = M(   33,   34);
EvalScore MINOR_ATTACK_ROOK             = M(   37,   30);
EvalScore MINOR_ATTACK_QUEEN            = M(   26,   42);
EvalScore ROOK_ATTACK_QUEEN             = M(   35,   19);

EvalScore mobilityKnight[9] = {
    M(  -63,   53), M(  -51,  104), M(  -46,  133), M(  -42,  150), M(  -37,  161),
    M(  -32,  172), M(  -26,  174), M(  -16,  168), M(   -3,  152), };

EvalScore mobilityBishop[14] = {
    M(  -15,   -3), M(   -4,   60), M(    4,   90), M(    8,  109), M(   12,  124),
    M(   16,  136), M(   17,  144), M(   15,  148), M(   17,  152), M(   20,  151),
    M(   27,  145), M(   43,  136), M(   51,  147), M(   77,  114), };

EvalScore mobilityRook[15] = {
    M(  -61,  142), M(  -54,  183), M(  -51,  219), M(  -50,  246), M(  -49,  262),
    M(  -43,  270), M(  -37,  277), M(  -29,  281), M(  -23,  287), M(  -18,  294),
    M(  -13,  299), M(   -9,  303), M(    3,  300), M(   34,  280), M(   92,  247), };

EvalScore mobilityQueen[28] = {
    M( -188,  186), M( -171,  186), M( -158,  298), M( -153,  371), M( -151,  410),
    M( -151,  437), M( -149,  460), M( -147,  475), M( -145,  487), M( -143,  494),
    M( -140,  500), M( -138,  503), M( -136,  504), M( -136,  508), M( -135,  510),
    M( -136,  510), M( -136,  508), M( -136,  507), M( -133,  502), M( -124,  491),
    M( -116,  477), M( -112,  466), M( -112,  454), M(  -94,  436), M( -147,  453),
    M(  -25,  365), M(  -79,  431), M( -131,  481), };

EvalScore hangingEval[5] = {
    M(   -3,   -2), M(   -4,   -1), M(   -6,   -7), M(   -5,   -5), M(   -4,   -8), };

EvalScore pinnedEval[15] = {
    M(    3,   -4), M(  -10,   10), M(   -8,   55), M(  -20,  -63), M(  -19,  -17),
    M(  -19,   49), M(   -2,  -11), M(  -27,  -15), M(  -15,   36), M(  -13,  -11),
    M(    6,   -8), M(  -14,   39), M(   13,  -23), M(  -11,  -29), M(  -17,   55), };

EvalScore passer_rank_n[N_RANKS] = {
    M(    0,    0), M(  -16,    5), M(  -25,   20), M(  -17,   62),
    M(   14,  103), M(   35,  173), M(   24,   85), M(    0,    0), };

EvalScore candidate_passer[N_RANKS] = {
    M(    0,    0), M(  -24,    6), M(   -7,   15), M(   -4,   28),
    M(    2,   83), M(    3,   76), M(    0,    0), M(    0,    0), };

EvalScore bishop_pawn_same_color_table_o[9] = {
    M(  -43,   71), M(  -48,   71), M(  -47,   58),
    M(  -49,   47), M(  -52,   36), M(  -56,   23),
    M(  -58,    5), M(  -57,  -12), M(  -64,  -56), };

EvalScore bishop_pawn_same_color_table_e[9] = {
    M(  -32,   50), M(  -49,   61), M(  -54,   54),
    M(  -60,   47), M(  -63,   39), M(  -66,   26),
    M(  -66,    9), M(  -64,   -2), M(  -73,  -12), };

int kingSafetyAttackWeights[6]{3, 32, 25, 35, 26, 0, };

int kingSafetyQueenExists         = -327;
...
EvalScore Evaluator::computePawns(Board* b){
    ...
    for(Color color:{WHITE, BLACK}){
        if(evalData.kingZone[!color] & evalData.pawnWestAttacks[color]){
            evalData.ksAttackCount [!color] += bitCount(evalData.kingZone[!color] &
                                                        evalData.pawnWestAttacks[color]);
            evalData.ksAttackWeight[!color] += kingSafetyAttackWeights[PAWN];
        }
        if(evalData.kingZone[!color] & evalData.pawnEastAttacks[color]){
            evalData.ksAttackCount [!color] += bitCount(evalData.kingZone[!color] &
                                                        evalData.pawnEastAttacks[color]);
            evalData.ksAttackWeight[!color] += kingSafetyAttackWeights[PAWN];
        }
    }
...
template<Color color> EvalScore Evaluator::computeKings(Board* b) {
    ...
    if(evalData.kingZone[!color] & evalData.attacks[color][KING]){
        evalData.ksAttackCount [!color] += bitCount(evalData.kingZone[!color] &
                                                    evalData.attacks  [color][KING]);
        evalData.ksAttackWeight[!color] += kingSafetyAttackWeights[KING];
    }
