...
EvalScore mobilityKnight[9] = {
        M(  -79,   -3), M(  -69,   40), M(  -64,   65), M(  -60,   79), M(  -56,   87),
        M(  -52,   96), M(  -47,   96), M(  -39,   90), M(  -27,   75), };

EvalScore mobilityBishop[14] = {
        M(  -14,  -14), M(   -5,   39), M(    2,   65), M(    5,   81), M(    9,   92),
        M(   12,  102), M(   13,  109), M(   12,  112), M(   12,  115), M(   15,  113),
        M(   20,  108), M(   34,  100), M(   38,  109), M(   60,   81), };

EvalScore mobilityRook[15] = {
        M(  -70,   56), M(  -64,   92), M(  -61,  123), M(  -61,  146), M(  -59,  160), 
        M(  -54,  167), M(  -49,  173), M(  -43,  176), M(  -39,  181), M(  -35,  187),
        M(  -31,  192), M(  -28,  195), M(  -18,  193), M(   12,  174), M(   64,  144), };

EvalScore mobilityQueen[28] = {
        M( -196,  129), M( -182,  136), M( -171,  235), M( -168,  299), M( -166,  334),
        M( -165,  358), M( -164,  378), M( -162,  391), M( -160,  402), M( -158,  409),
        M( -156,  415), M( -154,  420), M( -152,  423), M( -152,  428), M( -152,  431),
        M( -153,  432), M( -152,  431), M( -152,  429), M( -150,  426), M( -141,  414),
        M( -135,  402), M( -129,  391), M( -132,  380), M( -115,  362), M( -176,  385),
        M(  -48,  285), M( -129,  365), M( -141,  385), };

EvalScore hangingEval[5] = {
        M(   -3,   -1), M(   -3,    0), M(   -4,   -5), M(   -4,   -4), M(   -2,   -4), };

EvalScore pinnedEval[15] = {
        M(    2,   -6), M(   -9,    6), M(   -9,   40), M(  -19,  -53), M(  -16,  -16),
        M(  -18,   39), M(   -4,  -11), M(  -25,  -15), M(  -21,   38), M(  -10,  -13),
        M(   -3,   -8), M(  -14,   29), M(   10,  -33), M(   15,  -54), M(  -17,   41), };

EvalScore passer_rank_n[N_RANKS] = {
        M(    0,    0), M(  -12,    4), M(  -20,   18), M(  -14,   53),
        M(   12,   88), M(   32,  145), M(    5,   58), M(    0,    0), };

EvalScore candidate_passer[N_RANKS] = {
        M(    0,    0), M(  -23,    6), M(   -6,   13), M(   -2,   22),
        M(    3,   68), M(    8,   62), M(    0,    0), M(    0,    0), };

EvalScore bishop_pawn_same_color_table_o[9] = {
        M(  -51,   29), M(  -56,   31), M(  -55,   18),
        M(  -57,   10), M(  -59,    1), M(  -63,  -10),
        M(  -64,  -25), M(  -64,  -38), M(  -69,  -76), };

EvalScore bishop_pawn_same_color_table_e[9] = {
        M(  -43,   28), M(  -58,   36), M(  -61,   27),
        M(  -66,   20), M(  -68,   11), M(  -71,   -1),
        M(  -72,  -17), M(  -69,  -28), M(  -76,  -41), };

int KING_SAFETY_WEAK_SQUARES = 63;

int KING_SAFETY_NO_ENEMY_QUEEN = -184;
...
int KING_SAFETY_KNIGHT_DEFENDER = -29;

int kingSafetyAttackWeights[N_PIECE_TYPES]{0, 27, 13, 28, 16, 0, };
...
EvalScore Evaluator::computeKingSafety(Board* b) {
    ...
    int knightDefender = (evalData.attacks[color][KNIGHT] & evalData.kingZone[color]) != 0;
    ...
    int danger = evalData.ksAttackValue[color] * evalData.ksAttackCount[color] // attack power concept
                ...
                + (KING_SAFETY_KNIGHT_DEFENDER * knightDefender);
