...
void moveGen::addNoisy(Move m) {
    ...
    if (score >= 0) {
        score = 100000 + m_sd->getHistories(m, c, m_previous, m_followup, m_threatSquare) + score * 2 + 150 * (getSquareTo(m) == getSquareTo(m_previous));
        goodNoisyCount++;
    } else {
        score = 10000 + m_sd->getHistories(m, c, m_previous, m_followup, m_threatSquare) + score;
    }
    noisy[noisySize] = m;
