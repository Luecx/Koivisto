...
Move moveGen::next() {
    switch (stage) {
        ...
        case GEN_NOISY: generateNoisy(); stage++;
...
void moveGen::addNoisy(Move m) {
    ...
    int score  = (isPromotion(m) && (getPromotionPieceType(m) != QUEEN))
                     ? -1
                     : m_board->staticExchangeEvaluation(m);
...
void moveGen::addQuiet(Move m) {
    if (sameMove(m_hashMove, m) | sameMove(m_killer1, m) | sameMove(m_killer2, m))
        return;
    quiets[quietSize] = m;
    quietScores[quietSize++] =
        20000 + m_sd->getHistories(m, c, m_previous, m_followup, m_threatSquare);
}
...
bool moveGen::shouldSkip() { return m_skip; }