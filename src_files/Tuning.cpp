...
void tuning::findWorstPositions(Evaluator *evaluator, double K, int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        std::cout << boards[indexOfHighest]->fen() << " "
                  << evaluator->evaluate(boards[indexOfHighest]) << " " << results[indexOfHighest]
                  << std::endl;
    }
}
...
void tuning::generateHeatMap(Piece piece, bool earlyAndLate, bool asymmetric) {
    ...
    if (earlyAndLate && asymmetric) {
        ...
        for (int i = 0; i < dataCount; i++) {
            ...
            addToTable(earlyWhite,
                       earlyWhiteCount,
                       b->getPieces()[piece % 6],
                       whiteFactor * earlyChange,
                       WHITE);
            addToTable(lateWhite,
                       lateWhiteCount,
                       b->getPieces()[piece % 6],
                       whiteFactor * lateChange,
                       WHITE);
            addToTable(earlyBlack,
                       earlyBlackCount,
                       b->getPieces()[(piece % 6) + 6],
                       -whiteFactor * earlyChange,
                       WHITE);
            addToTable(lateBlack,
                       lateBlackCount,
                       b->getPieces()[(piece % 6) + 6],
                       -whiteFactor * lateChange,
                       WHITE);
...
void tuning::evalSpeed() {
    ...
    int ms = stopMeasure();
    std::cout << ms << "ms for " << dataCount << " positions = " << (1000 * dataCount / ms) / 1e6
              << "Mnps"
