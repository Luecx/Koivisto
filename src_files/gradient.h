...
struct ThreadData {
    ...
    Weight w_king_safety_unsafe_check;
    Weight w_hanging[5] {};
};
...
struct KingSafetyData {
    ...
    void   init(Board* b, EvalData* ev) {
        ...
        for (Color color: {WHITE, BLACK}) {
            ...
            unsafeChecks[color] = bitCount((rookChecks | bishopChecks | knightChecks) & ~vulnerable);
        }
    }
    ...
    void evaluate(float& midgame, float& endgame, ThreadData* td) {
        for (Color c : {WHITE, BLACK}) {
            ...
            danger[c] = attackValue * attackCount[c]
                        ...
                        + (td->w_king_safety_unsafe_check.midgame.value * unsafeChecks[c]);
    ...
    void gradient(float& mg_grad, float& eg_grad, ThreadData* td) {
        for (Color c : {WHITE, BLACK}) {
            ...
            td->w_king_safety_unsafe_check.midgame.gradient +=
                (danger_grad / 512) * (std::fmax(danger[c], 0) * unsafeChecks[c]);
            td->w_king_safety_unsafe_check.midgame.gradient +=
                (danger_grad / 32) * ((danger[c] > 0) * unsafeChecks[c]);
...
void                    load_weights() {
    for (int t = 0; t < N_THREAD; t++) {
        ...
        threadData[t].w_king_safety_unsafe_check = {KING_SAFETY_UNSAFE_CHECK, 0};
    }
}
...
void merge_gradients() {
    for (int t = 1; t < N_THREAD; t++) {
        ...
        threadData[t].w_king_safety_unsafe_check.merge(threadData[0].w_king_safety_unsafe_check);
    }
}
...
void share_weights() {
    for (int t = 1; t < N_THREAD; t++) {
        ...
        threadData[t].w_king_safety_unsafe_check.set(threadData[0].w_king_safety_unsafe_check);
    }
}
...
void adjust_weights(float eta) {
    ...
    threadData[0].w_king_safety_unsafe_check.update(eta);
...
void display_params() {
    ...
    std::cout << "int KING_SAFETY_UNSAFE_CHECK = ";
    std::cout << round(threadData[0].w_king_safety_unsafe_check.midgame.value) << ";\n" << std::endl;
