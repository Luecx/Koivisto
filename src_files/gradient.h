...
struct MetaData {
    ...
    float evaluate(float& mg, float& eg, ThreadData* td) {
        ...
        int pawnFactor = 8 - pawnCount[eg > 0 ? WHITE : BLACK];
        egReduction    = (td->w_pawn_reduction[0].endgame.value
                       - td->w_pawn_reduction[1].endgame.value * (pawnFactor * pawnFactor))
                      / 1024.0;
        eg *= egReduction;
    ...
    void gradient(float& mg_grad, float& eg_grad, float loss_grad, ThreadData* pData) {
        ...
        float egReduction_grad = rawEval[1] * eg_grad;
        pData->w_pawn_reduction[0].endgame.gradient += egReduction_grad / 1024.0;

        int pawnFactor = 8 - pawnCount[rawEval[1] > 0 ? WHITE : BLACK];
        pData->w_pawn_reduction[1].endgame.gradient -=
            egReduction_grad * (pawnFactor * pawnFactor) / 1024.0;
...
struct KingSafetyData {
                ...
                while (k) {
                    ...
                    if (c == WHITE) {
                        addToKingSafety(attacks, blackKingZone, bkingSafety_attPiecesCount,
                                        bkingSafety_valueOfAttacks, factors[p]);
                    } else {
                        addToKingSafety(attacks, whiteKingZone, wkingSafety_attPiecesCount,
                                        wkingSafety_valueOfAttacks, factors[p]);
                    }
    ...
    void evaluate(float& midgame, float& endgame, ThreadData* td) {
        midgame += td->w_king_safety[bkingsafety_index].midgame.value
                   - td->w_king_safety[wkingsafety_index].midgame.value;
        endgame += td->w_king_safety[bkingsafety_index].endgame.value
                   - td->w_king_safety[wkingsafety_index].endgame.value;
    }

    void gradient(float& mg_grad, float& eg_grad, ThreadData* td) {
        td->w_king_safety[bkingsafety_index].midgame.gradient += mg_grad;
        td->w_king_safety[bkingsafety_index].endgame.gradient += eg_grad;

        td->w_king_safety[wkingsafety_index].midgame.gradient -= mg_grad;
        td->w_king_safety[wkingsafety_index].endgame.gradient -= eg_grad;
    }
};
    ...
    void   init(Board* b, EvalData* ev) {
        ...
        for (int color = 0; color <= 1; color++) {
            ...
            while (potentialPinners) {
                ...
                // if there is exactly one of our pieces in the way, consider it pinned.
                // Otherwise, continue
                ...
                // add to the result indexing using pinnedPiece for which there are 5 different
                // pieces and the pinner
...
void                    load_weights() {
    for (int t = 0; t < N_THREAD; t++) {
        ...
        threadData[t].w_pawn_reduction[0] = {0, egPawnReductionA};
        threadData[t].w_pawn_reduction[1] = {0, egPawnReductionB};
    }
}
...
void merge_gradients() {
    for (int t = 1; t < N_THREAD; t++) {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            if (i < 2) {
                threadData[t].w_pawn_reduction[i].merge(threadData[0].w_pawn_reduction[i]);
            }
        }
    }
}
...
void share_weights() {
    for (int t = 1; t < N_THREAD; t++) {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            if (i < 2) {
                threadData[t].w_pawn_reduction[i].set(threadData[0].w_pawn_reduction[i]);
            }
        }
    }
}
...
void adjust_weights(float eta) {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        if (i < 2) {
            threadData[0].w_pawn_reduction[i].update(eta);
        }
    }
...
void display_params() {
    ...
    // --------------------------------- eg pawn reduction ---------------------------------
    std::cout << "int egPawnReductionA = " << round(threadData[0].w_pawn_reduction[0].endgame.value) << ";\n";
    std::cout << "int egPawnReductionB = " << round(threadData[0].w_pawn_reduction[1].endgame.value) << ";\n";
