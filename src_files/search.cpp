...
void                     search::initLMR() {
    ...
    for (d = 0; d < 256; d++)
        for (m = 0; m < 256; m++)
            lmrReductions[d][m] = 0.75 + log(d) * log(m) * 100 / search::LMR_DIV;
}
 ...
 */
void search::enableInfoStrings() { printInfo = true; }

/**
 * enables uci info-string printing. This is usually enabled but might be disabled for fen-generation.
 */
void search::disableInfoStrings() { printInfo = false; }

/**
 * clears the hash of the transposition table which is used for searches.
 */
void search::clearHash() { table->clear(); }

/**
 * prefetches the table
 * @param zobrist
 */
void search::prefetchHash(U64 zobrist) { __builtin_prefetch(&table->m_entries[zobrist & table->m_mask]); }
 ...
 * @return
 */
bool isTimeLeft() { return timeManager->isTimeLeft(); }

/**
 * checks if there is root time left and the iterative deepening should continue.
 * @return
 */
bool rootTimeLeft() { return timeManager->rootTimeLeft(); }

/**
 * used to change the hash size
 * @param hashSize
 */
void search::setHashSize(int hashSize) { table->setSize(hashSize); }
...
void printInfoString(Board* b, Depth d, Score score) {
    ...
    std::cout << " nodes " << nodes << " nps " << nps << " time " << timeManager->elapsedTime() << " hashfull "
        ...
        if (getSquareFrom(m) == sqFrom && getSquareTo(m) == sqTo) {
            if ((promo == 6 && !isPromotion(m)) || (isPromotion(m) && promo < 6 && promotionPiece(m) % 8 == promo)) {
                ...
                std::cout <<
                    ...
                    " nodes " << 1 << " nps " << 1 << " time " << timeManager->elapsedTime() << " hashfull "
                          << static_cast<int>(table->usage() * 1000);
                std::cout << std::endl;
...
/**
 * returns an searchOverview of the search which is internally used for various reasons.
 */
...
Move                   search::bestMove(Board* b, Depth maxDepth, TimeManager* timeMan, int threadId) {
    ...
    for (d = 1; d <= maxDepth; d++) {
        ...
        if (d < 6) {
            s = search::pvSearch(&searchBoard, -MAX_MATE_SCORE, MAX_MATE_SCORE, d, 0, td, 0);
        } else {
            Score window = 10;
            ...
            while (isTimeLeft()) {
                s = search::pvSearch(&searchBoard, alpha, beta, d, 0, td, 0);
 ...
 * @return
 */
Score search::pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td, Move skipMove,
                       Depth* lmrFactor) {
    UCI_ASSERT(b);
    UCI_ASSERT(td);
    ...
    if (b->getPreviousMove() == 0 && ply != 0) {
        ...
        staticEval = inCheck ? -MAX_MATE_SCORE + ply
                             : sd->evaluator.evaluate(b, alpha, beta) * ((b->getActivePlayer() == WHITE) ? 1 : -1);
    }
    ...
    while (moveOrderer.hasNext()) {
        ...
        if (!extension && depth >= 8 && !skipMove && legalMoves == 0 && sameMove(m, hashMove) && ply > 0
            && en.zobrist == zobrist && abs(en.score) < MIN_MATE_SCORE && (en.type == CUT_NODE || en.type == PV_NODE)
            && en.depth >= depth - 3) {
...
Score search::qSearch(Board* b, Score alpha, Score beta, Depth ply, ThreadData* td, bool inCheck) {
    ...
    for (int i = 0; i < mv->getSize(); i++) {
        ...
        if (see_piece_vals[(getCapturedPiece(m) % 8)] - see_piece_vals[(getMovingPiece(m) % 8)] - 300 + stand_pat
            > beta)
            return beta;
