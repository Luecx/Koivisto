...
Score Search::pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td,
    ...
    if (!skipMove && !inCheck && !pv) {
        ...
        // *******************************************************************************************
        // threat pruning:
        // if the static evaluation is already above beta at depth 1 and we have strong threats,
        // assume that we can atleast achieve beta
        // *******************************************************************************************
        if (   depth      == 1
...
Score Search::qSearch(Board* b, Score alpha, Score beta, Depth ply, ThreadData* td, bool inCheck) {
    ...
    for (int i = 0; i < mv->getSize(); i++) {
        ...
        Score see = (!inCheck
                     && (isCapture(m)
                         || isPromotion(m))) ? b->staticExchangeEvaluation(m) : 0;
        if (see < 0)
            continue;
