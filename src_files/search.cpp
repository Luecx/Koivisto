...
Score pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td, Move skipMove, Depth* lmrFactor) {
    ...
    while (moveOrderer.hasNext()) {
        ...
        Depth lmr = 0;(legalMoves == 0 || depth <= 2 || (isCapture(m) && staticExchangeEval >= 0)
        ...
        if ((legalMoves > 1 && depth >= 2) && 
        (!((isCapture(m) && staticExchangeEval >= 0) || (isPromotion && (promotionPiece(m) % 8 == QUEEN))) || lmrFactor!=nullptr)) {
