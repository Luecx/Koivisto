...
Score pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td, Move skipMove) {
    ...
    if (b->getPreviousMove() == 0 && ply != 0) {
        ...
        staticEval = -sd->eval[1 - b->getActivePlayer()][ply - 1] +eval::evaluateTempo(b) * 2;
    } else {
        staticEval =
            inCheck ? -MAX_MATE_SCORE + ply : eval::evaluate(b) * ((b->getActivePlayer() == WHITE) ? 1 : -1);
    }
...
Score qSearch(Board* b, Score alpha, Score beta, Depth ply, ThreadData* td) {
    ...
    if (b->getPreviousMove() == 0 && ply != 0) {
        ...
        stand_pat = -sd->eval[1 - b->getActivePlayer()][ply - 1] + eval::evaluateTempo(b) * 2;
    } else {
        stand_pat =
            inCheck ? -MAX_MATE_SCORE + ply : eval::evaluate(b) * ((b->getActivePlayer() == WHITE) ? 1 : -1);
    }
