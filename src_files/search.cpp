...
Score pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td, Move skipMove) {
    ...
    if (!skipMove && !inCheck && !pv) {
        ...
        // **********************************************************************************************************
        if (staticEval >= beta && !hasOnlyPawns(b, b->getActivePlayer()) && depth > 3) {
            b->move_null();
            score = -pvSearch(b, -beta, 1 - beta, depth - (depth / 4 + 3) * ONE_PLY -
                         (staticEval-beta<300 ? (staticEval-beta)/FUTILITY_MARGIN : 3), ply + ONE_PLY, td, 0);
            b->undoMove_null();
            if (score >= beta) {
                return beta;
            }
