 ...
 * @return
 */
Score pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td, Move skipMove, int lastLmrSide, Depth* lmrFactor) {
    UCI_ASSERT(b);
    UCI_ASSERT(td);
    ...
    if (!skipMove && !inCheck && !pv) {
        ...
        if (staticEval >= beta + (5 > depth ? 30 : 0) && !hasOnlyPawns(b, b->getActivePlayer())) {
            b->move_null();
            score = -pvSearch(b, -beta, 1 - beta, depth - (depth / 4 + 3) * ONE_PLY - (staticEval-beta<300 ? (staticEval-beta)/FUTILITY_MARGIN : 3), ply + ONE_PLY, td, 0, lastLmrSide);
            b->undoMove_null();
            if (score >= beta) {
                return beta;
            }
    ...
    while (moveOrderer.hasNext()) {
        ...
        if (!extension && depth >= 8 && !skipMove && legalMoves == 0 && sameMove(m, hashMove) && ply > 0
            ...
            score         = pvSearch(b, betaCut - 1, betaCut, depth >> 1, ply, td, m, lastLmrSide);
            if (score < betaCut) {
                ...
                score     = pvSearch(b, beta - 1, beta, (depth >> 1)+1, ply, td, m, lastLmrSide);
                if (score>=beta)
                    return score;
            }
            generateMoves(b, mv, hashMove, sd, ply);
