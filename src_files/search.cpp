...
U64 getNewThreats(Board* b, move::Move m) {
    ...
    switch (p) {
        case QUEEN:
            return 0;
        case ROOK:
            attacks =  lookUpRookAttacks(sqTo, occupied)
                    & ~lookUpRookAttacks(sqFrom, occupied);
            return attacks & (b->getPieceBB(!color, QUEEN));
        case BISHOP:
            attacks =  lookUpBishopAttacks(sqTo, occupied)
                    & ~lookUpBishopAttacks(sqFrom, occupied);
            return attacks & (b->getPieceBB(!color, QUEEN)
                            | b->getPieceBB(!color, ROOK));
        case KNIGHT:
