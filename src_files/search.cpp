...
bool tactical(Board *b, Move m, Color stm) {
    ...
    if (pieceType == PAWN)
        return  (((shiftEast(ONE << getSquareFrom(m)) | shiftWest(ONE << getSquareFrom(m)))) & b->getPieceBB(stm, PAWN))
                && (stm == WHITE ? (shiftNorthEast(ONE << getSquareTo(m)) | shiftNorthWest(ONE << getSquareTo(m))) :
                (shiftSouthEast(ONE << getSquareTo(m)) | shiftSouthWest(ONE << getSquareTo(m)))
                ) & (b->getTeamOccupiedBB(1 - stm) & ~b->getPieceBB(1 - stm, PAWN));
    if (pieceType == KNIGHT)
        return KNIGHT_ATTACKS[getSquareTo(m)] & (b->getPieceBB(1 - stm, ROOK) | b->getPieceBB(1 - stm, QUEEN));
    return false;
}
