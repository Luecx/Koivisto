...
Score pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td, Move skipMove) {
    ...
    if (!skipMove && !inCheck && !pv) {
        ...
        // **********************************************************************************************************
        Depth nullMoveReduction = (depth / 4 + 3) * ONE_PLY + (staticEval-beta<300 ? (staticEval-beta)/100 : 3);
        if ((staticEval >= beta || depth > 1+nullMoveReduction) && !hasOnlyPawns(b, b->getActivePlayer())) {
            b->move_null();
            score = -pvSearch(b, -beta, 1 - beta, depth - nullMoveReduction, ply + ONE_PLY, td, 0);
            b->undoMove_null();
            if (score >= beta) {
                return beta;
            }
