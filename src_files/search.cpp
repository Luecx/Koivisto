...
nn::DenseNetwork*        neuralNetworkEval = nullptr;
double                   neuralNetworkEvalAmount = 0;
...
void search_setNetwork(nn::DenseNetwork* network) { neuralNetworkEval = network; }

void search_setNetworkInfluence(double v){
    neuralNetworkEvalAmount = v;
}
...
Score pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td, Move skipMove) {
    ...
    Score staticEval = -MAX_MATE_SCORE + ply;
    if(!inCheck){
        staticEval = sd->evaluator.evaluate(b) * ((b->getActivePlayer() == WHITE) ? 1 : -1);
        if(neuralNetworkEval != nullptr){
            neuralNetworkEval->resetNetworkInput(b);
            Score nnEval = 100 * neuralNetworkEval->compute<false>();
            staticEval = staticEval * (1 - neuralNetworkEvalAmount) + nnEval * neuralNetworkEvalAmount;
        }
    }
...
Score qSearch(Board* b, Score alpha, Score beta, Depth ply, ThreadData* td) {
    ...
    if(!inCheck){
        ...
        if(neuralNetworkEval != nullptr){
            neuralNetworkEval->resetNetworkInput(b);
            Score nnEval = 100 * neuralNetworkEval->compute<false>();
            stand_pat = stand_pat * (1 - neuralNetworkEvalAmount) + nnEval * neuralNetworkEvalAmount;
        }
    }
