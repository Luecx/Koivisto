...
Score Search::pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td,
    ...
    if (depth == 0 || depth > MAX_PLY || ply > MAX_PLY) {
    ...
    if (!skipMove && !inCheck && !pv) {
        ...
        // **********************************************************************************************************
        if (staticEval >= beta + (5 > depth ? 30 : 0)  && !(ply > 1 && sd->playedMoves[ply - 2] == 0) && !(depth < 5 && enemyThreats > 0)
            && !(hashMove && en.type & ALL_NODE) && !hasOnlyPawns(b, b->getActivePlayer())) {
            b->move_null();
