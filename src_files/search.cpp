    ...
    while ((m = mGen->next())) {
        ...
        if (ply > 0 && legalMoves >= 1 && highestScore > -MIN_MATE_SCORE) {
            ...
            if (moveDepth <= 5 + quiet * 3
                ...
                && (isCapture(m) ? mGen->lastSee : b->staticExchangeEvaluation(m)) <= (quiet ? -40 * moveDepth : -100 * moveDepth) + 200 * (3 > chebyshevDistance(getSquareTo(m), bitscanForward(b->getPieceBB(!b->getActivePlayer(), KING)))))
                continue;
        }
