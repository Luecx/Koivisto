    ...
    while (moveOrderer.hasNext()) {
        ...
        if (ply > 0 && legalMoves >= 1 && highestScore > -MIN_MATE_SCORE) {
            ...
            if (quiet) {
                ...
                // move
                // **************************************************************************************************
                if (depth <= 7 && quiets > lmp[isImproving][depth] && !(getSquareFrom(m) == getSquareTo(sd->killer[!b->getActivePlayer()][ply + 1][0]) && sd->getHistories(m, b->getActivePlayer(), b->getPreviousMove()) > 0)) {
