    ...
    while ((m = mGen->next())) {
        ...
        if (ply > 0 && legalMoves >= 1 && highestScore > -MIN_MATE_SCORE) {
            ...
            if (quiet) {
                ...
                if (depth <= 7 && quiets >= lmp[isImproving][depth] && highestScore > std::min(alpha, staticEval)) {
                    mGen->skip();
                }
                ...
                if (!inCheck
                    && sd->getHistories(m, b->getActivePlayer(), b->getPreviousMove(),
                           ...
                           < std::min(140 - 30 * (depth * (depth + isImproving)), 0) && highestScore > std::min(alpha, staticEval)) {
                    continue;
                }
            }
