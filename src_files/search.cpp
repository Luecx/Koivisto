...
void getThreats(Board* b, SearchData* sd, Depth ply) {
    ...
    sd->threatCount[ply][WHITE] = bitCount(whitePawnAttacks);
    sd->threatCount[ply][BLACK] = bitCount(blackPawnAttacks);
    ...
    whiteMinorAttacks &= (b->getPieceBB<BLACK>(ROOK) | b->getPieceBB<BLACK>(QUEEN));
    blackMinorAttacks &= (b->getPieceBB<WHITE>(ROOK) | b->getPieceBB<WHITE>(QUEEN));

    sd->threatCount[ply][WHITE] += bitCount(whiteMinorAttacks);
    sd->threatCount[ply][BLACK] += bitCount(blackMinorAttacks);
 ...
 * @return
 */
Move Search::bestMove(Board* b, Depth maxDepth, TimeManager* p_timeManager, int threadId) {
    UCI_ASSERT(b);
    UCI_ASSERT(p_timeManager);
    ...
    if (threadId == 0) {
        ...
        this->timeManager = p_timeManager;
        ...
        for (int n = 1; n < threadCount; n++) {
            this->runningThreads.emplace_back(&Search::bestMove, this, b, maxDepth, p_timeManager, n);
        }
    }
    ...
    if (threadId == 0) {
        ...
        this->searchOverview.time  = p_timeManager->elapsedTime();
...
Score Search::pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td,
    ...
    if (!skipMove && !inCheck && !pv) {
        ...
        // **********************************************************************************************************
        if (    depth      == 1
            &&  staticEval >  beta + (isImproving ? 0 : 30)
            && !enemyThreats)
            return beta;
        ...
        if (   staticEval >= beta + (5 > depth ? 30 : 0)
            ...
            // do the null moves
            b->move_null();
            // store the history of moves
            sd->playedMoves[ply] = NULL_MOVE;
            score =
            ...
            // undo the null
            b->undoMove_null();
            // if we exceed beta, even after not doing anything, our position is too good
            if (score >= beta) {
                return score;
            }
        }
    ...
    mGen->init(sd, b, ply, hashMove, b->getPreviousMove(), ply > 1 ? sd->playedMoves[ply - 2] : 0,
               PV_SEARCH, mainThreat,
                    BISHOP_ATTACKS[kingSq][0] |
                    ROOK_ATTACKS  [kingSq][0] |
    ...
    while ((m = mGen->next()) != NO_MOVE) {
        ...
        if (ply > 0 && legalMoves >= 1 && highestScore > -MIN_MATE_SCORE) {
            ...
            if (quiet) {
                ...
                if (!inCheck
                    && moveDepth <= 7
                    && sd->maxImprovement[getSqToSqFromCombination(m)]
                               + moveDepth * FUTILITY_MARGIN
                               + 100
                ...
                // move.
                // ***********************************************************************************
                auto historyScore = sd->getHistories(m,
                                                     b->getActivePlayer(),
                                                     b->getPreviousMove(),
                                                     ply > 1 ? sd->playedMoves[ply - 2] : 0,
                                                     mainThreat);
                auto historyLimit = std::min(140 - 30 * (depth * (depth + isImproving)), 0);
                if (!inCheck
                    && historyScore < historyLimit) {
                    continue;
                }
            }
        ...
        if (depth >= 8
            ...
            mGen->init(sd, b, ply, hashMove, b->getPreviousMove(), ply > 1 ? sd->playedMoves[ply - 2] : 0, PV_SEARCH, mainThreat,
                             BISHOP_ATTACKS[kingSq][0]
                           | ROOK_ATTACKS  [kingSq][0]
                           | KNIGHT_ATTACKS[kingSq]);
            m = mGen->next();
        ...
        // quickly extract the node count here, used later
        ...
        // *******************************************************************************************
        Depth lmr = 0;
        if(!(legalMoves < 2 - (hashMove != 0) + pv                      // dont do lmr on first few moves
             || depth <= 2                                              // dont do lmr at low depth
             || (isCapture(m) && staticExchangeEval > 0)                // dont do lmr on winning captures
             || (isPromotion && (getPromotionPieceType(m) == QUEEN)))){ // dont do lmr on queen proms
        ...
        if (lmr) {
            ...
            lmr += !isImproving;
            lmr -= pv;
            lmr += !sd->targetReached;
            lmr += (sd->reduce && sd->sideToReduce != b->getActivePlayer());
            lmr -= std::min(1,sd->isKiller(m, ply, b->getActivePlayer()));

            if ( lmr       > MAX_PLY
                || history > 512
                || history > 256 && isCapture(m)) {
                lmr = 0;
            }
            lmr = std::min(lmr, (Depth)(depth-2));
        }
    
        // doing the move
        b->move(m);
        // store the move in some history list
        sd->playedMoves[ply] = m;
        // prefetch (relevent for multithreading)
        __builtin_prefetch(&table->m_entries[b->getBoardStatus()->zobrist & table->m_mask]);
    
    
        // adjust the extension policy for checks. we could use the givesCheck value but it has not
        // been validated to work 100%
        // if there is no extension and the depth is large enough while we are in check, extend
        if (   extension == 0
               && depth     > 4
        ...
        // if the current move is the hash move while in a non-pv node and the stored node type
        // inside the pv is an ALL_NODE or a FORCED_ALL_NODE or FORCED_CUT_NODE, we extend by 1
        ...
        // recursion logic for pvs. first move is searched with wider window
        // (or zero window if we are already in a zero-window)
        if (legalMoves == 0) {
        ...
        // compute how many nodes we used for this move
        U64 nodes_spend = td->nodes - nodeCount;
...
Score Search::qSearch(Board* b, Score alpha, Score beta, Depth ply, ThreadData* td, bool inCheck) {
    ...
    // compute stand pat. use a mate score if we are in check
    ...
    while ((m = mGen->next()) != NO_MOVE) {
        ...
        // *******************************************************************************************
        // compute the static exchange evaluation first
        Score see = 0;
        if(!inCheck && (isCapture(m) || isPromotion(m))){
            see = b->staticExchangeEvaluation(m);
        }
        // see skipping (dont look at bad captures)
        if (see < 0)
            continue;
        ...
        // prefetch (to increase smp performance)
        __builtin_prefetch(&table->m_entries[b->getBoardStatus()->zobrist & table->m_mask]);
    
        // check if the opponent is in check after we do the move
        ...
        // similar to pvs above,
        if (score > bestScore) {
            bestScore = score;
            bestMove  = m;
            if (score >= beta) {
                ttNodeType = CUT_NODE;
                // store the move with higher depth in tt in case the same capture would improve on
...
void           Search::clearHistory() {
    for (int i = 0; i < threadCount; i++) {
        if(this->tds[i] != nullptr) {
            memset(this->tds[i]->searchData.th, 0,             2*64*4096*4);
            memset(this->tds[i]->searchData.captureHistory, 0, 2*4096*4);
            memset(this->tds[i]->searchData.cmh, 0,            384*2*384*4);
            memset(this->tds[i]->searchData.fmh, 0,            384*2*384*4);
            memset(this->tds[i]->searchData.killer, 0,         2*257*2*4);
            memset(this->tds[i]->searchData.maxImprovement, 0, 64*64*4);
        }
    }
}
...
void Search::printInfoString(Board* b, Depth d, Score score) {
    ...
    if (abs(score) > MIN_MATE_SCORE) {
        std::cout   << " score mate "
                    << (MAX_MATE_SCORE - abs(score) + 1) / 2 * (score > 0 ? 1 : -1);
    } else {
...
Score Search::probeWDL(Board* board) {
    ...
    unsigned res = tb_probe_wdl(board->getTeamOccupiedBB()[WHITE],
...
Move Search::probeDTZ(Board* board) {
    ...
    unsigned result =
                 tb_probe_root (board->getTeamOccupiedBB()[WHITE],
                                board->getTeamOccupiedBB()[BLACK],
                                board->getPieceBB()[WHITE_KING  ] | board->getPieceBB()[BLACK_KING  ],
                                board->getPieceBB()[WHITE_QUEEN ] | board->getPieceBB()[BLACK_QUEEN ],
                                board->getPieceBB()[WHITE_ROOK  ] | board->getPieceBB()[BLACK_ROOK  ],
