...
Score pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td, Move skipMove, Depth* lmrFactor) {
    ...
    if (!skipMove && !inCheck && !pv) {
        ...
        // **********************************************************************************************************
        Depth R = (depth / 4 + 3) * ONE_PLY + (staticEval-beta<300 ? (staticEval-beta)/FUTILITY_MARGIN : 3);
       if (lmrFactor != nullptr) {
            if (*lmrFactor > R) R = 0;
            else R -= *lmrFactor;
        }
        if (staticEval >= beta && !hasOnlyPawns(b, b->getActivePlayer()) && R > 1) {
            b->move_null();
            score = -pvSearch(b, -beta, 1 - beta, depth - R, ply + ONE_PLY, td, 0);
            b->undoMove_null();
            if (score >= beta) {
                return beta;
            }
