...
Score Search::pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td,
    ...
    while ((m = mGen->next())) {
        ...
        // *********************************************************************************************************
        if (depth >= 8 && !skipMove && legalMoves == 0 && sameMove(m, hashMove) && ply > 0 && !inCheck && en.type == CUT_NODE && en.avgE != 0 && en.avgE * (1 - 2 * b->getActivePlayer()) < alpha - 50) {
            extension = 1;
        }
        ...
        if (score >= beta) {
            if (!skipMove && !td->dropOut) {
                ...
                if (sd->totalEvalCalls > callSoFar + 10)
                    averageEvals = (sd->totalEval - evalSoFar) / (sd->totalEvalCalls - callSoFar);
