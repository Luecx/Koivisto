...
bool tactical(Board *b, Move m, Color stm) {
    ...
    if (pieceType == KNIGHT)
        return !((stm == WHITE ? (shiftNorthEast(ONE << getSquareTo(m)) | shiftNorthWest(ONE << getSquareTo(m))) :
                (shiftSouthEast(ONE << getSquareTo(m)) | shiftSouthWest(ONE << getSquareTo(m)))) & b->getPieceBB(1 - stm, PAWN))
                 && bitCount(KNIGHT_ATTACKS[getSquareTo(m)] & (b->getPieceBB(1 - stm, ROOK) | b->getPieceBB(1 - stm, QUEEN))) > 1;
    return false;
}
